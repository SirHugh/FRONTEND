          ...
          {facturas.map((factura) => (
            <React.Fragment key={factura.id_comprobante}>
              <Table.Row
                ...
                className={`hover:bg-blue-200 bg-white border cursor-pointer justify-center ${showDetail === factura.id_comprobante ? "" : ""}`}
              >
                <Table.Cell>{factura.id_comprobante}</Table.Cell>
                <Table.Cell>{factura.nro_factura}</Table.Cell>
                <Table.Cell>{factura.cliente?.nombre} {factura.cliente?.apellido}</Table.Cell>
                <Table.Cell>{DateFormatter(new Date(factura.fecha))}</Table.Cell>
                <Table.Cell>{String(factura.hora).substring(0, 5)}</Table.Cell>
                <Table.Cell>{factura.tipo_pago === "C" ? "Contado" : "Credito"}</Table.Cell>
                <Table.Cell>{factura.forma_pago || "No especificado"}</Table.Cell>
                <Table.Cell>{CurrencyFormatter(Number(factura.monto))}</Table.Cell>
                <Table.Cell>                    
                  <Link to={`/printFactura/${factura.id_comprobante}`}>
                    <RiBillFill />
                  </Link>
                </Table.Cell>
              </Table.Row>
              <Table.Row className={`${showDetail === factura.id_comprobante ? "" : "hidden"} transition-all`}>
                          ...
                          <Table.Body>
                            {factura.aranceles.map((arancel, index) => (
                              <Table.Row className="bg-white border" key={index}>
                                ...
                                <Table.Cell>{CurrencyFormatter(Number(arancel.monto))}</Table.Cell>
                              </Table.Row>
                            ))}
                          </Table.Body>
                        </Table>
                          ...
                          <Table.Body>
                            {factura.ventas.map((pago, index) => (
                              <Table.Row className="bg-white border" key={index}>
                                ...
                                <Table.Cell>{CurrencyFormatter(Number(pago.monto))}</Table.Cell>
                              </Table.Row>
                            ))}
                          </Table.Body>
                        </Table>
