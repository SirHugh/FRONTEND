  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const matriculacionesPorGrado = await Promise.all(
          grados.map(async (grado) => {
            const matriculasResponse = await getMatricula(
              currentYear,
              grado.id_grado,
              "",
              1
  ...
  return (
    <div className="p-6">
      <h2 className="text-xl font-bold mb-4">
        Cantidad de Alumnos por Grado año {currentYear}
      </h2>
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={data} colors={colors}>
          ...
          <YAxis />
          <Tooltip />
          <Bar dataKey="cantidad" fill="#113485" />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
